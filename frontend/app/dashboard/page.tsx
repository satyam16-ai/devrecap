...
function DashboardContent() {
    ...
    const handleShare = async () => {
        ...
        setLoading(true);
        try {
            // Ensure styles/fonts are loaded
            await document.fonts.ready;
            ...
            if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    // title: 'My Developer Recap', // Some browsers ignore title if files present
                    // text: `Check out my GitHub stats! Generated by DevRecap.`,
                    files: [file]
                });
            } else {
                // Fallback
                alert("Native sharing not supported. Image will be downloaded.");
    ...
    const shareToPlatform = (platform: 'whatsapp' | 'twitter' | 'instagram') => {
        ...
        if (platform === 'whatsapp') {
            ...
            setLoading(true);
            // Force Story format for IG
            const prevFormat = exportFormat;
            if (exportFormat !== 'story') {
                // We can't easily switch state synchronously and capture immediately without re-render wait
                // So we just download current view but warn user
            }

            downloadCard().then(() => {
                setLoading(false);
                const confirmed = confirm("Image saved to your device! 📸\n\nOpen Instagram to share it to your Story?");
                if (confirmed) {
